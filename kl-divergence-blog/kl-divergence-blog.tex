...
\begin{defn}
    ...
    (or $-\int p\log p$ for continuous distributions, called the differential entropy).
\end{defn}
...
Hence $\H(f)\le\log(b-a)$. The inequality is an equality if and only if $f=u$ a.e.

\subsection{Cross-Entropy}
In the context of machine learning, we often approximate an unknown, underlying true distribution $p$ with a model $q$. Suppose we again want to measure the expected surprisal when sampling from the distribution. Samples are still drawn from $p$, but this time we can only measure surprisal with our known, approximating distribution $q$.
...
\begin{defn}
    ...
    (or $-\int p\log q$ for continuous distributions).
\end{defn}


\section{Kullback-Leibler Divergence}
\subsection{Motivation and Definition}
How much extra surprisal do we get by using our incorrect model $q$ instead of the true distribution $p$?
